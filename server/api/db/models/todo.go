...
var TodoWhere = struct {
	...
	ID:        whereHelperint{field: "\"grpc_stady_schema\".\"todo\".\"id\""},
	Title:     whereHelperstring{field: "\"grpc_stady_schema\".\"todo\".\"title\""},
	Context:   whereHelperstring{field: "\"grpc_stady_schema\".\"todo\".\"context\""},
	CreatedAt: whereHelpertime_Time{field: "\"grpc_stady_schema\".\"todo\".\"created_at\""},
	UpdatedAt: whereHelpertime_Time{field: "\"grpc_stady_schema\".\"todo\".\"updated_at\""},
}
...
var (
	todoAllColumns            = []string{"id", "title", "context", "created_at", "updated_at"}
	todoColumnsWithoutDefault = []string{"id", "title", "context", "created_at", "updated_at"}
...
func Todos(mods ...qm.QueryMod) todoQuery {
	mods = append(mods, qm.From("\"grpc_stady_schema\".\"todo\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"grpc_stady_schema\".\"todo\".*"})
	}
...
func FindTodo(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Todo, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"grpc_stady_schema\".\"todo\" where \"id\"=$1", sel,
	)
...
func (o *Todo) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"grpc_stady_schema\".\"todo\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"grpc_stady_schema\".\"todo\" %sDEFAULT VALUES%s"
		}
...
func (o *Todo) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"grpc_stady_schema\".\"todo\" SET %s WHERE %s",
...
func (o TodoSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"grpc_stady_schema\".\"todo\" SET %s WHERE %s",
...
func (o *Todo) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"grpc_stady_schema\".\"todo\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *Todo) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"grpc_stady_schema\".\"todo\" WHERE \"id\"=$1"
...
func (o TodoSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"grpc_stady_schema\".\"todo\" WHERE " +
...
func (o *TodoSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"grpc_stady_schema\".\"todo\".* FROM \"grpc_stady_schema\".\"todo\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, todoPrimaryKeyColumns, len(*o))
...
func TodoExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"grpc_stady_schema\".\"todo\" where \"id\"=$1 limit 1)"
